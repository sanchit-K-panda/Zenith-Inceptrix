...
type TabId = 'timetable' | 'notes' | 'assignments' | 'attendance' | 'ai-assistant'

// ============ Tab Configuration ============
const tabConfig: { id: TabId; label: string; icon: typeof Calendar }[] = [
...
function AnimatedTabContent({ children, tabKey }: AnimatedTabContentProps) {
  return (
    <motion.div
      key={tabKey}
...
export default function StudentDashboardLayout() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [ttRes, attRes, marksRes, assignRes, notesRes] = await Promise.allSettled([
      ...
      if (attRes.status === 'fulfilled') {
        ...
        const records = attData?.attendance || attData?.records || attData
        setAttendance(Array.isArray(records) ? records : [])
        setAttendanceStats(attData?.statistics || null)
      }

      // Handle marks
      if (marksRes.status === 'fulfilled') {
        setMarks(marksRes.value?.data?.statistics || null)
      }

      // Handle assignments
      if (assignRes.status === 'fulfilled') {
        const assignData = assignRes.value?.data
  ...
  const refreshNotes = () => {
    notesAPI.getStudentNotes().then((res) => {
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      case 'timetable':
        return (
          <AnimatedTabContent tabKey="timetable">
      ...
      case 'assignments':
        return (
          <AnimatedTabContent tabKey="assignments">
            <AssignmentsTab
      ...
      case 'attendance':
        return (
          <AnimatedTabContent tabKey="attendance">
            <AttendanceTab attendance={attendance} statistics={attendanceStats || undefined} />
          </AnimatedTabContent>
        )
      case 'ai-assistant':
        return (
          <AnimatedTabContent tabKey="ai-assistant">
            <AIAssistantTab />
          </AnimatedTabContent>
        )
      default:
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        >
          <motion.div variants={cardVariants}>
            <Card 
              className="border-l-4 border-l-blue-500 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
          ...
          <motion.div variants={cardVariants}>
            <Card 
              className="border-l-4 border-l-violet-500 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
          ...
          <motion.div variants={cardVariants}>
            <Card 
              className="border-l-4 border-l-amber-500 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
