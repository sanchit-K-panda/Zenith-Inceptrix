...
class TimetableOptimizer {
  ...
  async detectConflicts(): Promise<Conflict[]> {
    ...
    timetables.forEach(slot => {
      const key = `${slot.teacher._id}-${slot.dayOfWeek}-${slot.startTime}-${slot.endTime}`;
      
      if (teacherSchedule.has(key)) {
        conflicts.push({
          type: 'teacher_clash',
          classes: [slot, teacherSchedule.get(key)],
          description: `Teacher ${slot.teacher.firstName} has clash: ${slot.subject} and ${teacherSchedule.get(key).subject}`
        });
      }
